...
class Menu extends Component {
    ...
    render() {
        let { cataData, showStatus } = this.props;
        let items = this.getItems(cataData);//获取分类列表
        return (<View className='topiclist-menu'>
            <View style='position:absolute'>
                <AtDrawer onClose={this.closeDrawer.bind(this)} onItemClick={this.clickCata.bind(this)} show={showStatus} items={items}></AtDrawer>
            </View>
